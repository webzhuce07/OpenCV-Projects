...
{
	double d = -20 * log10(std::sqrt(std::pow(10, -7) + std::pow(10, -8.4)));
	std::cout << d;
    ...
    String modelTxt = "./resources/pose_deploy.prototxt";
    String modelBin = "./resources/pose_iter_102000.caffemodel";
	
	string pattern = "./resources/./*.PNG";
	vector<cv::String> imageFiles;
	cv::glob(pattern, imageFiles, false);
	size_t count = imageFiles.size();
    //String imageFiles = "2.PNG";
    ...
    if (parser.get<bool>("help") || modelTxt.empty() || modelBin.empty() || imageFiles.empty())
    {
	...
	for (size_t k = 0; k < count; k++) {
		Mat img = imread(imageFiles[k]);
		if (img.empty())
		{
			std::cerr << "Can't read image from the file: " << imageFiles[k] << std::endl;
			exit(-1);
		}

		cv::Mat outImage;
		cv::pyrMeanShiftFiltering(img, outImage, 10, 10);
		cv::imwrite("ms.png", outImage);
